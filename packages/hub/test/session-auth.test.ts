...
describe("认证失败限速（独立 Hub 实例，避免污染其他用例的计数）", () => {
  ...
  it("限速按 X-Forwarded-For 分桶：反代后不同客户端互不牵连", async () => {
    // 上一用例已把 socket 地址（127.0.0.1）打满；带 XFF 的请求按转发 IP 计数，
    // 攻击者（1.2.3.4）打满后主人（5.6.7.8）仍可通过——反代形态不退化为全局锁
    for (let i = 0; i < 10; i++) {
      const res = await fetch(`http://127.0.0.1:${hub.port}/auth/check`, {
    ...
    const attacker = await fetch(`http://127.0.0.1:${hub.port}/auth/check`, {
      headers: { authorization: `Bearer ${TOKEN}`, "x-forwarded-for": "1.2.3.4" },
    });
    expect(attacker.status).toBe(429);

    const owner = await fetch(`http://127.0.0.1:${hub.port}/auth/check`, {
      headers: { authorization: `Bearer ${TOKEN}`, "x-forwarded-for": "5.6.7.8" },
    });
